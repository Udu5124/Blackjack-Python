...
class Deck:
    ...
    def __str__(self):
        all_items = ''  # start with an empty string
        for item in self.deck:
            all_items = all_items + '\n ' + item.__str__()  # add each Card object's print string
        return all_items
...
def take_bet(chips):
    while True:
        try:
            chips.bet = int(input(f'How many chips would you like to bet? (Max : {chips.total})'))
        except ValueError:
...
while True:
    ...
    player_chips = Chips()  # default value is 1000
